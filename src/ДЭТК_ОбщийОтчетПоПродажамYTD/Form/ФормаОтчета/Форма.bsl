...
Процедура СформироватьОтчетНаСервере()
	...
	////расшифровка
	Область.Параметры.РасшифровкаМесяца = Новый Структура("Организация, Контрагент, ДатаНач, ДатаКон",
		Организация, Неопределено, Неопределено, Неопределено);
	...
	"ВЫБРАТЬ
	|	ВТ.Период КАК Период,
	|	СпрК.Ссылка КАК Контрагент,
	|	ЕстьNULL(СУММА(ВТ.Себестоимость),0) КАК Себестоимость,
	|	ЕстьNULL(СУММА(ВТ.ОбъемПродаж),0) КАК ОбъемПродаж
	|ИЗ
	|	ВТПродажи КАК ВТ
	|	left join Справочник.Контрагенты КАК СпрК
	|	ПО СпрК.Ссылка = ВТ.Контрагент
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ.Период,
	|	СпрК.Ссылка
	...
	Пока Выборка.Следующий() Цикл
		...
		ТоталПоКонтрагентуСС = 0;
		ТоталПоКонтрагентуОП = 0;
		...
		Пока Выборка2.Следующий() Цикл
			...
			//для колонки ИТОГО (по контрагенту)
			ТоталПоКонтрагентуСС=ТоталПоКонтрагентуСС+?(Выборка2.Себестоимость=Null, 0, Выборка2.Себестоимость);
			ТоталПоКонтрагентуОП=ТоталПоКонтрагентуОП+?(Выборка2.ОбъемПродаж=Null, 0, Выборка2.ОбъемПродаж);
		...
		Область.Параметры.ОбъемПродаж = ТоталПоКонтрагентуОП;
		Область.Параметры.Себестоимость = ТоталПоКонтрагентуСС;
		Попытка
			Область.Параметры.Рентабельность = ?(ТоталПоКонтрагентуСС=0, 0, (ТоталПоКонтрагентуОП-ТоталПоКонтрагентуСС)/ТоталПоКонтрагентуСС*100);
		Исключение
		КонецПопытки;
	...
	ТоталСС = ТЗПериодов.Итог("СебестоимостьТотал");
	ТоталОП = ТЗПериодов.Итог("ОбъемПродажТотал");
	
	Область.Параметры.ОбъемПродаж = ТоталОП;
	Область.Параметры.Себестоимость = ТоталСС;
	Попытка
		Область.Параметры.Рентабельность = ?(ТоталСС=0, 0, (ТоталОП-ТоталСС)/ТоталСС*100);
	Исключение
	КонецПопытки;
...
Процедура ТабРезультатОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	...
	Если ЭтоВнешнийОтчет() Тогда
		ОткрытьФорму("ВнешнийОтчет.ДЭТК_ОбщийОтчетПоПродажамYTD.Форма.ФормаОтчета", Расшифровка)
	Иначе
		ОткрытьФорму("Отчет.ДЭТК_ОбщийОтчетПоПродажамYTD.Форма.ФормаОтчета", Расшифровка)
	КонецЕсли;
